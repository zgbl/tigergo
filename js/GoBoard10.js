...
function moveForward() {
    if (currentMoveIndex < currentMoves.length - 1) {
        ...
        if (!move.pass) {
            placeStone(move.row, move.col, move.color, stoneSize);
        }
        updateMoveInfo();
        updateMoveDisplay();
    }
...
function handleFileSelect(event) {
    ...
    if (file) {
        ...
        reader.onload = function(e) {
            ...
            const parsedMoves = parseSGF(sgfContent);
...
/*
  function parseSGF2(sgfContent) {
    ...
    while ((match = regex.exec(sgfContent)) !== null) {
      const [, key, value] = match;
      if (key === 'B' || key === 'W') {
        //moves.push({ color: key, move: value });
        // 仅在value非空时推入moves
        if (value) {
            //moves.push({ color: key, move: value });
            const col = match[3].charCodeAt(0) - 97;
            ...
            console.log(`Warning: Move with key ${key} has an empty value`);
        }
...
/*function parseSGF2(sgfContent) {
    ...
    console.log("Line 847, blackPlayer:", info.PB);
    ...
    return {
        gameInfo: {
            blackPlayer: cleanValue(info.PB),  // 使用cleanValue函数去除括号
            whitePlayer: cleanValue(info.PW),  // 使用cleanValue函数去除括号
            blackRank: cleanValue(info.BR),    // 使用cleanValue函数去除括号
            whiteRank: cleanValue(info.WR),    // 使用cleanValue函数去除括号
            date: info.DT,
            result: info.RE,
            komi: info.KM,
            boardSize: info.SZ,
...
class SmallBoard {
    ...
    renderBoard(boardElement) {
        ...
        this.variationMoves.forEach((move, index) => {
            ...
            if (!move.pass) {
                //console.log("this 变化 move is:", move);
                //const [row, col] = move.split(',').map(Number);
                //this.placeStone(row, col, currentColor, index + 1);
...
function displayVariationComment(comment, originalMoves, variationMoves) {
    ...
    if (originalMoves && variationMoves) {
        ...
        prevButton.addEventListener('click', () => smallBoard.moveBackward());
        nextButton.addEventListener('click', () => smallBoard.moveForward());
    }
